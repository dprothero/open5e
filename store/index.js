...
export const actions = {
  LOAD_MONSTERS_LIST (context) {
    fetch.get(`${process.env.apiUrl}/monsters/?fields=slug,name,challenge_rating,type,size,hit_points,document__slug, document__title&limit=2000&ordering=slug`)
    .then(
      (response) => { context.commit('setMonstersList', response.data.results)}
    )
  },
  LOAD_SPELLS( context ) {
    fetch.get(`${process.env.apiUrl}/spells/?fields=slug,name,school,dnd_class,level,components,level_int, document__slug, document__title&limit=1000`) //you will need to enable CORS to make this work
    .then(
      (response => {
  ...
  LOAD_MAGICITEMS( context ) {
    fetch.get(`${process.env.apiUrl}/magicitems/?fields=slug,name,type,rarity&limit=1000`)
    .then(
      response => { context.commit( 'setMagicItemsList', response.data.results )
    })
  },
  LOAD_BACKGROUNDS( context ) {
    fetch.get(`${process.env.apiUrl}/backgrounds/?limit=1000`)
    .then(
      response => { context.commit( 'setBackgrounds', response.data.results )
    })
  },
  LOAD_CLASSES( context ) {
    fetch.get(`${process.env.apiUrl}/classes/`) //you will need to enable CORS to make this work
    .then(
  ...
  LOAD_RACES( context ) {
    if (!context.races) {
      fetch.get(`${process.env.apiUrl}/races/`) //you will need to enable CORS to make this work
      .then(
  ...
  LOAD_SECTIONS( context ) {
    if (!context.sections){
      fetch.get(`${process.env.apiUrl}/sections/`) //you will need to enable CORS to make this work
      .then(
